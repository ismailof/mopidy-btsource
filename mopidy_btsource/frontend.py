...
from .helper import format_play_time
...
class BTSourceFrontend(pykka.ThreadingActor, CoreListener):
        ...
        try:
            if self._is_bt_selected:
                if bt_track:
                    ...
                    logger.debug('BT Track changed. Updating stream title to %r',
                                 stream_title)
                    self._set_stream_title(stream_title)
                else:
                    self._set_stream_title(None)
            else:
                # Stop source bluetooth to avoid simultaneous playing
                self.bt_player.stop()
    ...
    def bt_position_changed(self, bt_position):
        if self._is_bt_selected:
            if bt_position is not None:
                logger.debug('BT Position changed. Updating position to %s',
                             format_play_time(bt_position))
    ...
    def _refresh_bt_position(self, core_position=None):
        ...
        if core_position is None:
            ...
            if abs(bt_position - core_position) > 1000:
                logger.debug('Correcting mopidy core position %s to BT position: %s',
                             format_play_time(core_position),
                             format_play_time(bt_position))
                self._update_core_position(bt_position)
    ...
    def _set_stream_title(self, title):
        # For now there is no method to update the value of stream_title in playback
